...
class AnimationWidget(QGLWidget):
    ...
    def compile_city_drawing(self):
        ...
        for (i, j) in self.black_cells:
            self.draw_rect(i*self.cell_size, j*self.cell_size,
        ...
        for (i, j) in self.white_cells:
            glColor3f(0.0, 0.0, 0.0)
            self.draw_outline(i*self.cell_size, j*self.cell_size,
                              self.cell_size, self.cell_size)
    ...
    def create_GLU(self):
        ...
        glutInitDisplayMode(GLUT_RGBA | GLUT_DOUBLE | GLUT_ALPHA | GLUT_DEPTH)
        # set window size
        glutInitWindowSize(self.WIN_WIDTH, self.WIN_HEIGTH)
        # set window position
    ...
    def update(self):
        ...
        self.next_vehicles = [
            v for v in self.vehicles if v.state in States.moving_states()]
