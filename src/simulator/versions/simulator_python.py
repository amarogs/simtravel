...
def configure_a_star( city_map,  city_size):
    global CITY, SIZE
    CITY = city_map
    SIZE = city_size
...
def  a_star( start,  goal):
    ...
    depth = 0 #Controls the number of entries in the path we want to compute
...
def  recompute_path( current_path,  pos,  target):
...
def  lattice_distance(pos1, pos2):
    global SIZE
