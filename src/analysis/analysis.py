...
class GraphFunctions():
    ...
    def get_grayscale_from(self, image):
        """Given a 2D numpy array, return a grayscale version of the array."""
        minimum = np.min(image)
        # Get rid of negative values
        shift = image + minimum 
        # Expan to a scale of 255
        return np.array((255/np.max(shift))*shift, dtype="uint8")
    def image_histogram_equalization(self,image, number_bins=256):
        """Given an image and a number of bins, creates a histogram of the image and then 
        equalizes the image by doing the proper transformation. This histogram equalization is based on 
        https://opencv-python-tutroals.readthedocs.io/en/latest/py_tutorials/py_imgproc/py_histograms/py_histogram_equalization/py_histogram_equalization.html.
        
         """
        
        hist, bins = np.histogram(image.flatten(), number_bins,[1,255] ,density=True)
        # Compute cumulative sum of the histogram masking the values equal to zero.
        cdf = hist.cumsum() 
    ...
    def graph_road_usage(self, heat_map_mean, base_heat_map_mean):
        """Given a dictionary called heat_map_mean = {k=snapshot number, val=matrix of occupation of cells.}
        and a the same dictionary for a base simulation. Takes the last snapshot contained in the dictionaries
        and returns a canvas with the difference in road usage."""
        def plot_histogram(high, low,title):
            plt.figure()
            plt.subplot(121)
            plt.hist(high.flatten()[high.flatten()!=0],256)
            plt.subplot(122)
            plt.hist(low.flatten()[low.flatten()!=0],256)
            
            plt.show()
        # Get the desired snapshot
        key = max([int(number) for number in heat_map_mean.keys()])
        # Create the normalization constant
        norm = 1.0/((key+1)*self.total_measures/len(heat_map_mean))

        # Retrieve the heat maps and normalize them
        heat_map = norm * heat_map_mean[str(key)]
        base_map = norm * base_heat_map_mean[str(key)]
        road_map = heat_map == 0

        # Compare both by doing a substraction
        diff_map = heat_map - base_map
        # Find slice points by performing a histogram analysis
        p = 0.15

        histogram, bins = np.histogram(diff_map[~road_map].flatten(), density=True)
        cdf = histogram.cumsum()/histogram.cumsum()[-1]
        # Find the p-percentile and 1-p percentile.
        first, second = [i for (i,val) in enumerate(cdf) if val>=p][0], [i for i,val in enumerate(cdf) if val >=(1-p)][0]

        slice_zero = (diff_map >=bins[first]) * (diff_map<=bins[second])
        
        
        negativa_values = -1 * (diff_map < 0)
        positive_values = diff_map >0
        sign_matrix = negativa_values + positive_values

        # diff_map = 0.0001*diff_map*diff_map*sign_matrix

        
        # source_map = self.get_grayscale_from(self.image_histogram_equalization(self.get_grayscale_from(diff_map)))
        # source_map[slice_zero] = 128
        source_map = self.get_grayscale_from(diff_map)
        # Mark the road cells as invalid values
        source_map = np.ma.masked_where(road_map, source_map)
        ...
        # Create the image from the array and the colormap
        img = canvas.axes.imshow(source_map,cmap=cmap, interpolation='none')
        cbar = canvas.fig.colorbar(img, ticks=[1,128, 255])
        cbar.ax.set_yticklabels(['Menos', 'Igual', 'Más'])
        # Set a title
        canvas.axes.set_title("Cambio en la utilización de las celdas")
        canvas.fig.tight_layout()
    ...
    def graph_heat_map_evolution(self, heat_map_mean,x=None, live=False):
        ...
        for (i, hmap) in heat_map_mean.items():
            ...
            canvas.axes = canvas.fig.add_subplot(111)
            canvas.axes.grid(False)
            canvas.axes.set_title(super_title.format(eval(i)+1, len(heat_map_mean)))
...
class SimulationAnalysis(Simulation):
    def __init__(self, EV_DEN, TF_DEN, ST_LAYOUT, PATH, filepath):
        ...
        self.base_sim_filepath = self.find_base_simulation(EV_DEN, TF_DEN, ST_LAYOUT,PATH, filepath)

        # load the data into the object
        self.load_data()
        # load the info from the base simulation
        self.load_base_heat_map()
    ...
    # def lambda33(self):
    #     return self.grapher.graph_road_usage(self.heat_map_mean, self.base_heat_map_mean)
    def lambda4(self):
        return self.grapher.graph_velocities_evolution(self.velocities_mean, self.velocities_std)
    def find_base_simulation(self,EV_DEN, TF_DEN, ST_LAYOUT,PATH, filepath):
        results_path = os.path.join(PATH, "results")
        candidates = []
        for candidate_file in os.listdir(results_path):
            if candidate_file[-4:]=="hdf5":
                
                name_splitted = candidate_file.split("#")
                
                
                if name_splitted[1] == TF_DEN and name_splitted[2]==ST_LAYOUT+".hdf5":
                    candidates.append(candidate_file)
        base = min(candidates, key=lambda x: x.split("#")[0] )
        
        return os.path.join(results_path, base)
    ...
    def load_base_heat_map(self):
        """Load certain attributes from the HDF5 file of the base simulation.
        The base simulation is used to compare to this one and all its attributes
        start with 'base_' """

        with h5py.File(self.base_sim_filepath, "r") as file:
            ...
            for group in list(file['0'].keys()):
                if group == "heat_map":
                    ...
                    self.__setattr__("base_"+group+"_mean", data_mean)
                    self.__setattr__("base_"+group+"_std", data_std)
